...
export class PostCommentService {
  ...
  public async createComment(
    ...
    const getStatus = await this.dataStatusModel.findOne({
      where: { status_number: comment.status_number },
    });
    if (!getStatus) {
      throw new NotFoundException(`${comment.status_number} not found`);
    } else {
      ...
      commentInput.status_id = getStatus.dataValues.id;
