      ...
      if (!userResults) {
        ...
        if (!eventResults) {
          ...
          const getStatus = await this.dataStatusModel.findOne({
            where: { status_number: ratings.status_number },
          });
          if (!getStatus) {
            throw new NotFoundException(`${ratings.status_number} not found`);
          } else {
            ...
            ratingsEntry.status_id = getStatus.dataValues.id;
